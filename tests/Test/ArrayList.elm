module Test.ArrayList exposing (tests)

import ElmTest exposing (..)

import ArrayList exposing (..)
...
testListOfN n =
  let xs = ArrayList.fromList [1..n]
      xsOpp = ArrayList.fromList [(-n)..(-1)]
      xsNeg = foldl cons empty xsOpp -- assume foldl and (::) work
      zs = fromList [0..n]
      sumSeq k = k * (k + 1) // 2
      xsSum = sumSeq n
      mid = n // 2
  in
      suite (toString n ++ " elements")
        ...
        , suite "map"
            ...
            , test "linear" <| assertEqual (fromList [2..(n + 1)]) (map ((+) 1) xs)
            ]
        ...
        , suite "filter"
            [ test "none" <| assertEqual (empty) (filter (\x -> x > n) xs)
            , test "one" <| assertEqual (fromList [n]) (filter (\z -> z == n) zs)
            , test "all" <| assertEqual (xs) (filter (\x -> x <= n) xs)
            ]

        , suite "take"
            [ test "none" <| assertEqual (empty) (take 0 xs)
            , test "some" <| assertEqual (fromList [0..(n - 1)]) (take n zs)
        ...
        , suite "drop"
            ...
            , test "some" <| assertEqual (fromList [n]) (drop n zs)
            , test "all" <| assertEqual (empty) (drop n xs)
            , test "all+" <| assertEqual (empty) (drop (n + 1) xs)
            ]
        ...
        , test "(::)" <| assertEqual (append (fromList [-1]) xs) (cons -1 xs)

        , test "concat" <| assertEqual (append xs (append zs xs)) (concat (fromList [xs, zs, xs]))
        ...
        , suite "partition"
            [ test "left" <| assertEqual (xs, empty) (partition (\x -> x > 0) xs)
            , test "right" <| assertEqual (empty, xs) (partition (\x -> x < 0) xs)
            , test "split" <| assertEqual (fromList [mid + 1..n], fromList [1..mid]) (partition (\x -> x > mid) xs)
            ]
        ...
        , suite "filterMap"
            [ test "none" <| assertEqual (empty) (filterMap (\x -> Nothing) xs)
            ...
            , let halve x =
              ...
              in
                  test "some" <| assertEqual (fromList [1..mid]) (filterMap halve xs)
            ]

        , suite "concatMap"
            [ test "none" <| assertEqual (empty) (concatMap (\x -> empty) xs)
            , test "all" <| assertEqual (xsNeg) (concatMap (\x -> fromList [-x]) xs)
            ]
        ...
        , test "scanl" <| assertEqual (cons 0 (map sumSeq xs)) (scanl (+) 0 xs)
        ]
